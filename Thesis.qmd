...
Puerto Rico faces acute demographic challenges, including a rapidly aging population, shrinking tax base, and institutional fiscal stress—that demand robust, forward-looking mortality projections. This thesis applies the Lee-Carter model to Puerto Rico's age and sex specific mortality data from 1950 to 2023, decomposing historical death rates into their baseline, sensitivity, and trend components. A bootstrapped simulation framework generates 500 probabilistic mortality trajectories, producing 40-year stochastic forecasts of age-specific death rates and life expectancy at birth with 95% confidence intervals. Results project median life expectancy at birth reaching 82 years for males and 94 years for females by 2062, with male projections broadly consistent with those of the UN World Population Prospects, the U.S. Census Bureau's International Data Base, and the IHME. Female projections, however, exhibit pronounced upward skew that exceeds best-practice benchmarks in the literature, a pattern traced to data irregularities in the 2010–2023 period rather than modeling error. Key structural findings include a plateau in young adult male mortality attributable to excess violence-related deaths and high sensitivity of infant and child mortality to the overall trend index. The wide confidence intervals produced are themselves a substantive finding, signaling that demographic uncertainty in Puerto Rico constitutes a genuine planning risk warranting scenario-based fiscal and healthcare policy rather than single-point forecasts. Future work should implement a hierarchical Bayesian model and investigate the underlying data quality issues driving the female projection skew.
...
fig_width <- 8
fig_height <- 4
ggsave("graphs/projected_log_mortality.png",plot = g, width=fig_width, height=fig_height)
...
ggsave("graphs/projected_life_expectancy.png",plot = g, width=fig_width, height=fig_height)
g
...
#| fig-height: 6
#| fig-width: 10
make_kable <- function(df, cols_per_section = 1) {
  # df must have columns: age, ax, bx, sex
  # cols_per_section: how many age-group sections to display side-by-side
  ...
  # Split by sex
  male   <- df |> filter(sex == "male")   |> select(age, ax, bx)
  female <- df |> filter(sex == "female") |> select(age, ax, bx)
  
  # Join on age so rows align
  combined <- left_join(male, female, by = "age", suffix = c("_m", "_f")) |>
    rename(ax_m = ax_m, bx_m = bx_m, ax_f = ax_f, bx_f = bx_f)
  
  n      <- nrow(combined)
  half   <- ceiling(n / 2)
  
  # Split into two halves to place side-by-side
  left_half  <- combined[1:half, ]
  right_half <- combined[(half + 1):n, ]
  
  # Pad right half if odd number of rows
  if (nrow(right_half) < nrow(left_half)) {
    pad <- left_half[1, ] |> mutate(across(everything(), ~NA))
    right_half <- bind_rows(right_half, pad)
  }
  
  # Build wide table: age | male(ax,bx) | female(ax,bx) || age | male(ax,bx) | female(ax,bx)
  wide <- bind_cols(left_half, right_half)
  
  # Column names (will be overridden by kable header)
  colnames(wide) <- c(
    "Age", "ax", "bx", "ax", "bx",
    "Age", "ax", "bx", "ax", "bx"
  )
  
  # Multi-level header spanning groups
  # Level 1: two big sections
  # Level 2: Age | Male | Female | Age | Male | Female
  wide |>
    kbl(
      booktabs  = TRUE,
      align     = c("c", "r", "r", "r", "r", "c", "r", "r", "r", "r"),
      linesep   = "",
       digits = sig_digits,
      caption = "Lee-Carter parameters ax and bx by age and sex"
    ) |>
    kable_styling(
    ...
    column_spec(6, border_left = TRUE)  # visual divider between sections
      }
threshold <- 100
df <- rio::import("graphs/01_ax_bx.csv") 
make_kable(df)|> kable_styling(full_width = TRUE, font_size = 8)
...
n <- as.integer(nrow(df)/2)
left <- df[1:n, ]
right <- df[(n+1):nrow(df), ]
wide <- bind_cols(left, right)
wide|> kable(
  col.names = c("Year", "Male kt", "Female kt", "Year", "Male kt", "Female kt"),
  align = c("c", "r", "r", "c", "r", "r"),
  booktabs = TRUE,
  caption = "Lee-Carter parameter kt by year and sex"
) |>
...
bootstrap_residuals_ax   |>
  mutate(
    val   = paste0(round(q_0500, sig_digits),   " (", round(q_0025, sig_digits),   ", ", round(q_0975, sig_digits),   ")"),
  ) |> rename(ax=val) |> select(age,ax,sex) |> pivot_wider(names_from = sex, values_from = ax) |> select(age, male, female) |> kable(
    col.names = c("Age", "Male ax (2.5%, 97.5%)", "Female ax (2.5%, 97.5%)"), caption = "Bootstrap confidence intervals for ax"   
...
bootstrap_residuals_bx   |>
  mutate(
    val   = paste0(round(q_0500, sig_digits),   " (", round(q_0025, sig_digits),   ", ", round(q_0975, sig_digits),   ")"),
  ) |> rename(bx=val) |> select(age,bx,sex) |> make_overflow_kable3(
    index_col = "age",
    value_cols = "bx",
    group_col = "sex",
    caption = "Bootstrap confidence intervals for bx"
  )
```
...
bootstrap_residuals_kt   |>
  mutate(
    val   = paste0(round(q_0500, sig_digits),   " (", round(q_0025, sig_digits),   ", ", round(q_0975, sig_digits),   ")"),
  ) |> rename(kt=val) |> select(year,kt,sex) |> make_overflow_kable3(
    index_col = "year",
    value_cols = "kt",
    group_col = "sex",
    caption = "Bootstrap confidence intervals for kt"
  )
```
...
bootstrap_residuals_kt   |>
  mutate(
    val   = paste0(round(q_0500, sig_digits),   " (", round(q_0025, sig_digits),   ", ", round(q_0975, sig_digits),   ")"),
  ) |> rename(kt=val) |> select(year,kt,sex) 
```
